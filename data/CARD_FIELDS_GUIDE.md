...
│   │   ├── content.md   # 文章内容
│   │   ├── cover.png   # 可选，卡片封面
│   │   └── background.png  # 可选，正文页背景图（支持 .png/.jpg/.webp）
│   └── 文章目录2/
│       ├── card.json
│       └── content.md
└── 分类目录2/
...
### 正文页背景图（可选）
- 在博客或项目**同目录下**放置文件名为 `background` 的图片，扩展名支持 `png`、`jpg`、`webp`（如 `background.png`）。
- 生成后，该博客/项目的正文页会以该图作为**整页背景**（含导航栏以外区域），并自动叠加半透明白色以保证可读；文章内容区域仍为白底。
- 若不放置该文件，整页使用默认浅灰背景。
